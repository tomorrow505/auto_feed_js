...
npm run dev
```
开发默认启动 **完整脚本** 服务器（端口 5174），适用于 Safari 等无法加载本地文件的浏览器：

```
http://localhost:5174/auto-feed-refactor.user.js
```
...
```
构建产物在 `dist/`：
- `dist/auto-feed-refactor.user.js`（完整脚本，Safari/生产用）
- `dist/auto-feed-loader.user.js`（本地 loader，Chrome 开发用）

## 使用说明
1. 生产/日常使用：直接安装 `dist/auto-feed-refactor.user.js`
2. 本地开发：默认使用完整脚本（5174），如需 loader 见 `npm run dev:loader`
3. 设置面板：Alt+S 打开脚本设置
